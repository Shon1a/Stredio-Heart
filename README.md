...
The crate is **pure and I/O-free** (`#![forbid(unsafe_code)]`): bytes in, bytes out. Network, storage, and rendering belong to the shell. It builds to a native `rlib` and to WebAssembly, so a browser can drive it directly.
...
| `collection` | Merge the official add-on list over the built-in defaults (display-only refine, additive-only, no-stream + XSS guards). |
...
Prebuilt WebAssembly glue is committed at [`web/`](web) and served via jsDelivr, so a shell can `import` it with no build step.
